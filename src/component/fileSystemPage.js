...
class FileSystemPage extends Component {
  ...
  componentDidMount(){
    ...
    this.podInterval = setInterval(() => {
      this.fetchFileList(this.state.currentDir);
  ...
  getHomeDir(){
    axios.get('http://9.30.97.77:10010/v1/listPath')
    .then(res => {
  ...
  scpFileTo(){
    ...
    axios.post("http://9.30.97.77:10010/v1/scpTo", config)
    .then(res =>{
      this.setState({
        modalStatus:'result',
      })
    ...
    return(
      <div>
        <Modal show={this.state.modalStatus!=='none'}>
          ...
          <Modal.Body>
            {this.state.modalStatus==='set'&&<ScpForm
              scpUserValue = {this.state.scpUser}
              scpPasswordValue = {this.state.scpPassword}
              scpIpValue = {this.state.scpIp}
              remotePathValue = {this.state.remotePath}
              changeScpUser={that.changeScpUser}
              changeScpPassword={that.changeScpPassword}
              changeScpIp={that.changeScpIp}
              changeRemotePath={that.changeRemotePath}
            />}
